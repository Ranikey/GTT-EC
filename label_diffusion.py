...
def homology_matrix(fasta, cutoff=0.1):
    ...
    os.system('{} makedb --in {} -d {} --quiet'.format(Diamond_PATH, fasta, fasta))
    print('Start homology alignment...')
    os.system('{} blastp -d {}.dmnd -q {} -o {} --very-sensitive --quiet -p 8'.format(Diamond_PATH, fasta, fasta, fasta + '.tsv'))
...
def LabelDiffusion(initial_pred, lamda, fasta, identity_cutoff=0.1):
    train_fasta = f'{_DATASETS}/Training_set_filter.fasta'
    test_fasta = fasta
    os.system('{} makedb --in {} -d {} --quiet'.format(Diamond_PATH, train_fasta, train_fasta))
    print('Start alignment to the training set...')
    os.system('{} blastp -d {}.dmnd -q {} -o {} --very-sensitive --quiet -p 8'.format(Diamond_PATH, train_fasta, test_fasta, 'testVStrain.tsv'))
    ...
    with open(f'{_DATASETS}/train_set.pkl', 'rb') as f:
